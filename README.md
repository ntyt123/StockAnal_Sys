...
- **A股数据自动下载**：系统启动时自动检查并下载A股基础数据，确保数据完整性
- **定时数据更新**：每天16:00自动更新股票信息，保持数据时效性
- **智能数据检查**：实时监控数据库状态，无数据时自动触发下载
- **手动数据同步**：支持手动触发数据下载，满足特殊需求
...
智能分析系统/
│
├── web_server.py            # Web服务器和路由控制（集成股票数据下载）
├── stock_analyzer.py        # 股票分析核心引擎
├── us_stock_service.py      # 美股服务（可选）
├── start.sh                 # 服务管理脚本
├── start_web_server.sh      # 集成启动脚本（推荐）
├── start_web_server.bat     # Windows启动脚本
├── news_fetcher.py          # 新闻获取与缓存
├── stock_qa.py              # 智能问答功能，支持联网搜索
├── stock_select/            # 股票选择工具目录
│   ├── get_stock_info.py    # A股数据下载器（独立版本）
│   ├── test_downloader.py   # 功能测试脚本
│   └── README.md            # 使用说明文档
│
├── templates/               # HTML模板
│   ├── layout.html          # 基础布局模板
